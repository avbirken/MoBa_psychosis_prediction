...
%% Get results
load('Z:/users/parekh/2024-11-11_predictionPsychosis_VB/work/2025-02-06_compiledMLResults_unimodal');
...
%             'Birth factors'};
mdlNames = cell(5, 1);
row1     = {'Psychotic',  'General',        'Schizophrenia',   'Parent and',           'Birth'};
row2     = {'experiences', 'mental health', 'PRS',     'childhood diagnoses',   'related factors'};
...
for res = 1:5
    plot(allH(1), [xloc1(res)-jitterAmount, xloc1(res)+jitterAmount], repmat(mean(test_balAccuracy_RUS(:,res) .* 100), 2, 1),      'LineWidth', 1, 'Color', col_line);
    plot(allH(1), [xloc2(res)-jitterAmount, xloc2(res)+jitterAmount], repmat(mean(test_balAccuracy_SMOTE_SVM(:,res) .* 100), 2, 1), 'LineWidth', 1, 'Color', col_line);
    plot(allH(1), [xloc3(res)-jitterAmount, xloc3(res)+jitterAmount], repmat(mean(test_balAccuracy_SVM_balAcc(:,res) .* 100), 2, 1), 'LineWidth', 1, 'Color', col_line);
end
