...
% Model 1: CAPE alone
res = 1;
scatter(allH(1), repmat(ticks(res), 50,  1), res_trimodal.mean_testBalAccuracy_1 .* 100,     marker_size, col_RUS,  'filled', marker_RUS,  'jitter', jitter, 'jitterAmount', jitterAmount);
scatter(allH(1), repmat(ticks(res), 100, 1), res_trimodal.perm_mean_testBalAccuracy_1 .*100, marker_size, col_perm, 'filled', marker_perm, 'jitter', jitter, 'jitterAmount', jitterAmount, 'MarkerEdgeColor', col_perm, 'LineWidth', 1);

% Model 2: Q14 alone
res = 2;
scatter(allH(1), repmat(ticks(res), 50,  1), res_trimodal.mean_testBalAccuracy_2 .* 100,     marker_size, col_RUS,  'filled', marker_RUS,  'jitter', jitter, 'jitterAmount', jitterAmount);
scatter(allH(1), repmat(ticks(res), 100, 1), res_trimodal.perm_mean_testBalAccuracy_2 .*100, marker_size, col_perm, 'filled', marker_perm, 'jitter', jitter, 'jitterAmount', jitterAmount, 'MarkerEdgeColor', col_perm, 'LineWidth', 1);

% Model 3: NPR alone
res = 3;
scatter(allH(1), repmat(ticks(res), 50,  1), res_trimodal.mean_testBalAccuracy_3 .* 100,     marker_size, col_RUS,  'filled', marker_RUS,  'jitter', jitter, 'jitterAmount', jitterAmount);
scatter(allH(1), repmat(ticks(res), 100, 1), res_trimodal.perm_mean_testBalAccuracy_3 .*100, marker_size, col_perm, 'filled', marker_perm, 'jitter', jitter, 'jitterAmount', jitterAmount, 'MarkerEdgeColor', col_perm, 'LineWidth', 1);

% Model 4: trimodal
res = 4;
scatter(allH(1), repmat(ticks(res), 50,  1), res_trimodal.mean_testBalAccuracy_F .* 100,     marker_size, col_SVM,  'filled', marker_SVM,  'jitter', jitter, 'jitterAmount', jitterAmount);
scatter(allH(1), repmat(ticks(res), 100, 1), res_trimodal.perm_mean_testBalAccuracy_F .*100, marker_size, col_perm, 'filled', marker_perm, 'jitter', jitter, 'jitterAmount', jitterAmount, 'MarkerEdgeColor', col_perm, 'LineWidth', 1);

% Add mean performances
res = 1;
plot(allH(1), [ticks(res)-jitterAmount, ticks(res)+jitterAmount], repmat(mean(res_trimodal.mean_testBalAccuracy_1 .* 100), 2, 1), 'LineWidth', 1, 'Color', col_line);
res = 2;
plot(allH(1), [ticks(res)-jitterAmount, ticks(res)+jitterAmount], repmat(mean(res_trimodal.mean_testBalAccuracy_2 .* 100), 2, 1), 'LineWidth', 1, 'Color', col_line);
res = 3;
plot(allH(1), [ticks(res)-jitterAmount, ticks(res)+jitterAmount], repmat(mean(res_trimodal.mean_testBalAccuracy_3 .* 100), 2, 1), 'LineWidth', 1, 'Color', col_line);
res = 4;
plot(allH(1), [ticks(res)-jitterAmount, ticks(res)+jitterAmount], repmat(mean(res_trimodal.mean_testBalAccuracy_F .* 100), 2, 1), 'LineWidth', 1, 'Color', col_line);
...
res = 4;
text(allH(1), ticks(res), 95, [num2str(mean(res_trimodal.mean_testBalAccuracy_F .* 100), '%05.2f'), ' ± ', num2str(std(res_trimodal.mean_testBalAccuracy_F .*100), '%05.2f')], 'HorizontalAlignment', 'center', 'FontSize', 9, 'Color', col_SVM);
...
res = 4;
text(allH(1), ticks(res), 30, ['Sens: ', num2str(mean(res_trimodal.mean_testSensitivity_F .* 100), '%05.2f'), ' ± ', num2str(std(res_trimodal.mean_testSensitivity_F .* 100), '%05.2f')], 'HorizontalAlignment', 'center', 'FontSize', fontsize_sens, 'Color', col_SVM);
text(allH(1), ticks(res), 25, ['Spec: ', num2str(mean(res_trimodal.mean_testSpecificity_F .* 100), '%05.2f'), ' ± ', num2str(std(res_trimodal.mean_testSpecificity_F .* 100), '%05.2f')], 'HorizontalAlignment', 'center', 'FontSize', fontsize_sens, 'Color', col_SVM);
...
%% Save
print('/ess/p697/cluster/users/parekh/2024-11-11_predictionPsychosis_VB/work/Multimodal_trimodal_CAPE-Q14-NPR.png', '-dpng', '-r900');
close(fig);