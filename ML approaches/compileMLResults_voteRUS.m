...
for ii = 1:length(listFiles)
    ...
    % Compile results
    if ii == 1
        results = compileResults(toLoad, modalityString{ii}, firstModality{ii}, secondModality{ii}, thirdModality{ii});
    else
        results = [results; compileResults(toLoad, modalityString{ii}, firstModality{ii}, secondModality{ii}, thirdModality{ii})];
    end
end
...
%% Save
writetable(results, '/ess/p697/cluster/users/parekh/2024-11-11_predictionPsychosis_VB/work/summary_results_vote_mode_RUSBoost.csv');
...
function results = compileResults(toLoad, fullName, firstModality, secondModality, thirdModality)
% Initialize
varNames = {'ModelName', 'ModalityName', 'nCases', 'nControls', 'nTrainCases', 'nTrainControls', 'nTestCases', 'nTestControls', ...
            'TP', 'FP', 'TN', 'FN', 'Sensitivity', 'Specificity', 'BalAccuracy', 'PermSensitivity', 'PermSpecificity', 'PermBalAccuracy'};
...
% TP, FP, TN, FN - vote
tmp_test_TP = ceil(mean(vote_test_TP));
tmp_test_TN = ceil(mean(vote_test_TN));
tmp_test_FP = ceil(mean(vote_test_FP));
tmp_test_FN = ceil(mean(vote_test_FN));

results{4,9}  = [num2str(ceil(mean(tmp_test_TP))), ' [', num2str(min(tmp_test_TP)), '-', num2str(max(tmp_test_TP)), ']'];
results{4,10} = [num2str(ceil(mean(tmp_test_FP))), ' [', num2str(min(tmp_test_FP)), '-', num2str(max(tmp_test_FP)), ']'];
results{4,11} = [num2str(ceil(mean(tmp_test_TN))), ' [', num2str(min(tmp_test_TN)), '-', num2str(max(tmp_test_TN)), ']'];
results{4,12} = [num2str(ceil(mean(tmp_test_FN))), ' [', num2str(min(tmp_test_FN)), '-', num2str(max(tmp_test_FN)), ']'];
...
% Sensitivity, specificity, balanced accuracy - vote
tmp_sensitivity = mean(vote_test_sensitivity) * 100;
tmp_specificity = mean(vote_test_specificity) * 100;
tmp_balAccuracy = mean(vote_test_balAccuracy) * 100;

results{4,13} = [num2str(mean(tmp_sensitivity), '%.02f'), ' ± ', num2str(std(tmp_sensitivity), '%.02f'), ' [', num2str(min(tmp_sensitivity), '%.02f'), '-', num2str(max(tmp_sensitivity), '%.02f'), ']'];
results{4,14} = [num2str(mean(tmp_specificity), '%.02f'), ' ± ', num2str(std(tmp_specificity), '%.02f'), ' [', num2str(min(tmp_specificity), '%.02f'), '-', num2str(max(tmp_specificity), '%.02f'), ']'];
results{4,15} = [num2str(mean(tmp_balAccuracy), '%.02f'), ' ± ', num2str(std(tmp_balAccuracy), '%.02f'), ' [', num2str(min(tmp_balAccuracy), '%.02f'), '-', num2str(max(tmp_balAccuracy), '%.02f'), ']'];
...
% Permutation sensitivity, specificity, balanced accuracy - vote
tmp_sensitivity = mean(perm_vote_test_sensitivity) * 100;
tmp_specificity = mean(perm_vote_test_specificity) * 100;
tmp_balAccuracy = mean(perm_vote_test_balAccuracy) * 100;

results{4,16} = [num2str(mean(tmp_sensitivity), '%.02f'), ' ± ', num2str(std(tmp_sensitivity), '%.02f'), ' [', num2str(min(tmp_sensitivity), '%.02f'), '-', num2str(max(tmp_sensitivity), '%.02f'), ']'];
results{4,17} = [num2str(mean(tmp_specificity), '%.02f'), ' ± ', num2str(std(tmp_specificity), '%.02f'), ' [', num2str(min(tmp_specificity), '%.02f'), '-', num2str(max(tmp_specificity), '%.02f'), ']'];
results{4,18} = [num2str(mean(tmp_balAccuracy), '%.02f'), ' ± ', num2str(std(tmp_balAccuracy), '%.02f'), ' [', num2str(min(tmp_balAccuracy), '%.02f'), '-', num2str(max(tmp_balAccuracy), '%.02f'), ']'];
end