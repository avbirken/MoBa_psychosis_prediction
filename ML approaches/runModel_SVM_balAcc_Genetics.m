...
dirOut  = '/ess/p697/cluster/users/parekh/2024-11-11_predictionPsychosis_VB/work/analysis_SVM_balAcc';
toLoad  = 'Genetics.mat';
toSave  = 'Results_SVM_balAcc_Genetics_SCZ.mat';
...
numOuterFolds   = 5;
numInnerFolds   = 5;
locFeatures     = find(not(cellfun(@isempty, regexpi(Genetics.Properties.VariableNames, '^SCZ_Pt'))));
locCovariates   = setdiff([find(not(cellfun(@isempty, regexpi(Genetics.Properties.VariableNames, '^PC')))), ...
                   find(strcmpi(Genetics.Properties.VariableNames, 'sex')), ...
                   find(not(cellfun(@isempty, regexpi(Genetics.Properties.VariableNames, '^Batch'))))], ...
                   find(strcmpi(Genetics.Properties.VariableNames, 'Batch_24')));
numObservations = size(Genetics,1);
optimizeHow     = 'balacc';
doOpt           = true;
innerSeeds      = [];
...
%% Find out which variables are contious
...
covarLocs   = 1:size(allFeatures,2);

%% Initialize
[trainPrd, trainWts, testPrd,      testWts,       ...
 cost,     lambda,   performanceVal] = deal(cell(numOuterFolds, numRepeats));
...
[perm_trainPrd, perm_trainWts, perm_testPrd,      perm_testWts,       ...
 perm_cost,     perm_lambda,   perm_performanceVal] = deal(cell(numOuterFolds, numPermutations));
...
parfor rep = 1:numRepeats
    ...
    for folds = 1:numOuterFolds
        ...
        % Optimize SVM, figure out parameters, train a model, and make
        % prediction using this trained model
        [trainPrd{folds,rep}, trainWts{folds,rep}, testPrd{folds,rep},  testWts{folds,rep}, ...
         cost{folds,rep},     lambda{folds,rep},   performanceVal{folds,rep}] =             ...
         doSVM(tmp_trainFeatures,   tmp_testFeatures, tmp_trainClasses,                     ...
                       tmp_trainCovars,     tmp_testCovars,   covarLocs,                            ...
                       numInnerFolds,       tmp_parentID,     stdLocs,                              ...
                       innerSeeds,          optimizeHow,      doOpt);
        ...
        [~, ~, ~, train_AUC(folds,rep)] = perfcurve(tmp_trainClasses, trainWts{folds,rep}(:,2), 1, 'Cost', cost{folds,rep});
        [~, ~, ~, test_AUC(folds, rep)] = perfcurve(tmp_testClasses,  testWts{folds,rep}(:,2),  1, 'Cost', cost{folds,rep});
...
parfor rep = 1:numPermutations
    ...
    for folds = 1:numOuterFolds
        ...
        [perm_trainPrd{folds,rep}, perm_trainWts{folds,rep}, perm_testPrd{folds,rep},  perm_testWts{folds,rep}, ...
         perm_cost{folds,rep},     perm_lambda{folds,rep},   perm_performanceVal{folds,rep}] =             ...
         doSVM(tmp_trainFeatures,   tmp_testFeatures, tmp_trainClasses,                     ...
                       tmp_trainCovars,     tmp_testCovars,   covarLocs,                            ...
                       numInnerFolds,       tmp_parentID,     stdLocs,                              ...
                       innerSeeds,          optimizeHow,      doOpt);
        ...
        % Calculate area under the curve
        [~, ~, ~, perm_train_AUC(folds,rep)] = perfcurve(tmp_trainClasses, perm_trainWts{folds,rep}(:,2), 1, 'Cost', perm_cost{folds,rep});
        [~, ~, ~, perm_test_AUC(folds, rep)] = perfcurve(tmp_testClasses,  perm_testWts{folds,rep}(:,2),  1, 'Cost', perm_cost{folds,rep});
